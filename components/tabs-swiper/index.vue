...
export default {
  ...
  methods: {
    ...
    swiperScrollLower() {
      uni.showToast({
        icon: "none",
        title: `此时为${this.tabs[this.swiperIndex].title}触底`,
      });
      setTimeout(() => {
        this.getData();
      }, 500);
    ...
    getData() {
      ...
      setTimeout(() => {
        for (let index = 0; index < 10; index++) {
          let arr = this.tabs[this.swiperIndex].content;
          this.$set(
            arr,
            arr.length,
            Math.random() + "-" + index + this.tabs[this.swiperIndex].title
          );
        }
        uni.hideLoading();
      }, 1000);
      console.log(this.tabs[this.swiperIndex]);
    },
    // 下拉事件
    handleRefre() {
      ...
      setTimeout(() => {
        this.tabs[this.swiperIndex].content = [];
        for (var i = 0; i < 5; i++) {
          this.tabs[this.swiperIndex].content.push([
            this.tabs[this.swiperIndex].title + "下拉-" + i,
          ]);
        }
        uni.hideLoading();
      }, 1000);
...
swiper-item {
  ...
  color: #000;
  position: relative;
  background-color: rgb(114, 118, 119);
  color: #fff;
}
